...
import { GalleryItemUpdateSchema } from '@/lib/validation/public';
...
export async function GET(_req: Request, { params }: Context) {
  ...
  try {
    ...
    if (!item) return apiError(404, 'NOT_FOUND', 'Gallery item not found.');
    return ok(item);
  } catch {
...
export async function PATCH(req: Request, { params }: Context) {
  ...
  if (auth.response) return auth.response;
  const { id } = await params;
  const parsed = GalleryItemUpdateSchema.safeParse(await readJsonBody(req));
  if (!parsed.success) return apiError(422, 'VALIDATION_ERROR', 'Invalid gallery item data.', parsed.error.flatten());

  try {
    const item = await prisma.$transaction(async (tx) => {
      const existing = await tx.galleryItem.findUnique({ where: { id }, select: { id: true } });
      if (!existing) return null;
      const updated = await tx.galleryItem.update({ where: { id }, data: parsed.data });
...
export async function DELETE(_req: Request, { params }: Context) {
  ...
  try {
    ...
    if (!deleted) return apiError(404, 'NOT_FOUND', 'Gallery item not found.');
    return ok({ id });
  } catch {
    return unexpectedError();
  }
